...
with st.popover(label = ":material/info:"):
    st.write(
        "Enter the GEO Series accession (GSE) of any dataset that is microarray or bulk RNAseq."
    )
...
if not re.match(r"^GSE\d+$", gse_id):
    st.error("Please enter a valid GEO Series accession (e.g. GSE183620).")
    st.stop()
...
def get_dp_and_char(selected_gpl):
    gpl_gsm_ids = meta["gsm_gpl_dict"].get(selected_gpl, [])

    if not gpl_gsm_ids:
        st.warning(f"No samples found for platform '{selected_gpl}'.")
        st.session_state["dp_text"] = ""
        st.session_state["char_df"] = pd.DataFrame()
        return

    first_gsm = GLOBAL_GSE.gsms[gpl_gsm_ids[0]]
    dp_text = first_gsm.metadata['data_processing'][0]
    st.session_state["dp_text"] = dp_text

    if "characteristics_ch1" not in first_gsm.metadata:
        st.warning("No characteristics found in the first sample. Characteristics table will be empty.")
        st.session_state["char_df"] = pd.DataFrame()
        return

    char_list = [x.split(": ")[0] for x in first_gsm.metadata["characteristics_ch1"]]
    char_df = pd.DataFrame(columns=char_list)
    for gsm in gpl_gsm_ids:
        try:
            ...
            st.warning(f"Skipping {gsm}: incomplete metadata ({e})")
            continue
